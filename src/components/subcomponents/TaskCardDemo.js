...
export default function TaskCard({ task, isAdmin, company }) {
  ...
  // --- POPRAWIONE ROZPOZNAWANIE DATY ---
  let dueDate = null;
  if (task.deadline) {
    if (typeof task.deadline.toDate === 'function') {
      // 1. Jeśli to pełny obiekt Firebase Timestamp
      dueDate = task.deadline.toDate();
    } else if (task.deadline.seconds) {
      // 2. Jeśli to "mockowy" obiekt JSON (dla dema)
      dueDate = new Date(task.deadline.seconds * 1000);
    } else if (typeof task.deadline === 'number') {
      // 3. Jeśli to same milisekundy (jak te nadawane przez TaskAdd w trakcie tworzenia)
      dueDate = new Date(task.deadline);
    } else {
      // Zabezpieczenie na wypadek stringów / innych date
      dueDate = new Date(task.deadline);
    }
  }

  const deadlineDate = dueDate && !isNaN(dueDate) ? dueDate.toLocaleDateString() : 'Brak daty';
  
  // Obliczenia pozostałych dni
  let daysRemaining = 0;
  if (dueDate) {
  ...
  const accept = () => {
    // DEMO: zamiast dzwonić do bazy, logujemy to do konsoli
    console.log(`DEMO: Usunięto zadanie ID: ${task.id} dla firmy: ${company}`);
    toast.current.show({ severity: 'info', summary: 'Potwierdzono (Demo)', detail: 'Usunąłeś zadanie', life: 3000 });
  }
    ...
    <li className={`taskCard ${deadlineClass}`}>
      ...
      <div className="taskCard-team">
        ...
        <div className='taskCard-team-body'>
          {task.team && task.team.length > 0 ? task.team.map((member, index) => (
